...
public class GalleryDao {
	...
	public void plusGallery(Gallery gallery) {
		String sql = "INSERT INTO GalleryList (galleryId, passwd, repreName, galleryName,"
				+ " galleryScale, galleryAddr, galleryNumber, galleryEmail, gallerySns)"
				 + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
		try {
			Connection con = null;
			...
			try {
				...
				pstmt.setString(3, gallery.getRepreName());
				pstmt.setString(4, gallery.getGalleryName());
				pstmt.setString(5, gallery.getGalleryScale());
				pstmt.setString(6, gallery.getGalleryAddr());
				pstmt.setString(7, gallery.getGalleryNumber());
				pstmt.setString(8, gallery.getGalleryEmail());
				pstmt.setString(9, gallery.getGallerySns());
				pstmt.executeUpdate();
			} finally {
		...
		try {
			...
			try {
				...
				while(rs.next()) {
					...
					gallery.setRepreName(rs.getString("repreName"));
	...
	public int idCheck(String id) {
		String sql = "SELECT userId FROM GalleryList WHERE galleryId = ?";	
		int value = 0;			
		try {
			Connection con = null;
			PreparedStatement pstmt = null;
